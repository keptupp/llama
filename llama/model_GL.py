...
class Attention(nn.Module):
    ...
    def forward(
        ...
        if mask is not None:
            scores = scores + mask.detach().clone()  # (bs, n_local_heads, seqlen, cache_len + seqlen)
...
class TransformerBlock(nn.Module):
    ...
    def forward(
        ...
        """
            ...
            因此，这里也需要对token的下面的词进行屏蔽，
            我想到的办法是加大padding的长度，使当前词的特征只能靠之前的toeken词得到，如卷积核为5的卷积，设置padding为4
...
class Transformer(nn.Module):
    ...
    def forward(self, tokens: torch.Tensor, start_pos: int):
        ...
        # 将可学习全局编码加入token中
        b,l,d=h.shape
        h=torch.cat([self.global_token.repeat(b,1,1),h],dim=1)
        seqlen+=self.token_nums#加上全局编码的长度
        start_pos+=self.token_nums
